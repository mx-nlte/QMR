...
#ifndef EIGEN_QMR_ReOrth_H
#define EIGEN_QMR_ReOrth_H
...
inline MatrixType matProd1(MatrixType &A, MatrixType &B){
	return A.adjoint() * B;
}

template<typename MatrixType, typename Rhs, typename Dest, typename Preconditioner>
bool qmr_reorth(const MatrixType & mat, const Rhs & rhs, Dest & x, const Preconditioner & precond,
	...
	for (Index i = 1; i <= maxIters; i++)
	{
		...
		// Reorthogonalization
		const int numReOrth = 1;

		for (Index n = 1; n <= numReOrth; n++)
		{
			for (Index m = 0; m < i; m++)
			{
				v -= W.col(m) * (W.col(m).adjoint()*v); 
				w -= V.col(m) * (V.col(m).adjoint()*w); 
				// abs1 = v.norm();
				// abs2 = w.norm();
				v = v/v.norm();
				w = w/w.norm();
...
struct traits<QMR_ReOrth<_MatrixType,_Preconditioner> >
{
...
/** \ingroup IterativeLinearSolvers_Module
  * \brief A QMR reorthogonolised solver for sparse square problems
  *
  ...
  * 
  * QMR reorthogonolised can also be used in a matrix-free context, see the following \link MatrixfreeSolverExample example \endlink.
  *
  * \sa class SimplicialCholesky, DiagonalPreconditioner, IdentityPreconditioner
  */
template< typename _MatrixType, typename _Preconditioner>
class QMR_ReOrth : public IterativeSolverBase<QMR_ReOrth<_MatrixType,_Preconditioner> >
{
  typedef IterativeSolverBase<QMR_ReOrth> Base;
  using Base::matrix;
  using Base::m_error;
...
public:
  ...
  explicit QMR_ReOrth(const EigenBase<MatrixDerived>& A) : Base(A.derived()), m_restart(30) {}
  ...
  void _solve_vector_with_guess_impl(const Rhs& b, Dest& x) const
  {
    ...
    bool ret = internal::qmr_reorth(matrix(), b, x, Base::m_preconditioner, m_iterations, m_restart, m_error);
